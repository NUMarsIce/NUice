 ...
 */
class NUDRV8701Driver : public NUDriver {
...
public:
     ...
     */
    NUDRV8701Driver(ros::NUNodeHandle& nh, const char* ns, uint8_t movement_board_number, uint8_t update_hz = 4) :
                NUDriver(nh, ns),
                dir_sub(appendNamespace("/set_dir"), &NUDRV8701Driver::dir_cb, this),
                speed_sub(appendNamespace("/set_speed"), &NUDRV8701Driver::speed_cb, this),
                en_sub(appendNamespace("/enable"), &NUDRV8701Driver::enable_cb, this),
    ...
    NUDRV8701Driver(ros::NUNodeHandle& nh, const char* ns, uint8_t cur_pin, uint8_t ph_pin, uint8_t en_pin, uint8_t nsleep_pin, uint8_t nfault_pin, uint8_t update_hz = 4) : 
                NUDriver(nh, ns),
                dir_sub(appendNamespace("/set_dir"), &NUDRV8701Driver::dir_cb, this),
                speed_sub(appendNamespace("/set_speed"), &NUDRV8701Driver::speed_cb, this),
                en_sub(appendNamespace("/enable"), &NUDRV8701Driver::enable_cb, this),
...
private:
    ...
    //Subscribers
    ros::Subscriber<std_msgs::Bool, NUDRV8701Driver> dir_sub;
    void dir_cb(const std_msgs::Bool& msg){
        digitalWrite(ph_pin_, msg.data);
    }

    ros::Subscriber<std_msgs::Bool, NUDRV8701Driver> en_sub;
    void enable_cb(const std_msgs::Bool& msg){
        digitalWrite(nsleep_pin_, msg.data);
    }

    ros::Subscriber<std_msgs::UInt8, NUDRV8701Driver> speed_sub;
